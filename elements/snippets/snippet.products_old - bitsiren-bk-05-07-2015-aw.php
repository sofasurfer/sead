	...
	if( !empty($bookingList) && count($bookingList) > 0 ){
		foreach( $bookingList as $bookingItem ){
			...
			foreach( $bookingItem['rates'] as $rateItem ){
				...
				$retHtml .= "<td style=\"text-align:center;\"><button title=\"remove from list\" onclick=\"bookingDeleteItem('".$bookingItem['id'] . "_" . $rateItem['id']."');\" class=\"delete\"></button></td>";
				$retHtml .= "</tr>";
...
*/
		...
		foreach($tours as $tourItem){		
			...
			if( !empty($sDeskArray) && !empty($sDeskArray[$languageCode])  ){
				...
				$listProperties['shortdescription'] = $sDeskArray[$languageCode];
                                ...
                                $listProperties['shortdescription'] = '&nbsp;';
                        }
	...
	if( !empty($sDeskArray) && !empty($sDeskArray[$languageCode])  ){
		...
		$tourDetailArray['shortdescription'] = $sDeskArray[$languageCode];
            ...
            $tourDetailArray['shortdescription'] ='';
        }
	...
	if( !empty($duration) ){
        ...
        if( !empty($pdfAttachment) && !empty($pdfAttachment[$languageCode]) ){
		...
		foreach( $ratesArray as $rateItem ){
			...
			if( empty($isAdmin) ){
				$title = "";
                        ...
                        //noom edited 03/09/2015.
                        
			$rKEy = 'liveaboard_' . $schedule->get('liveaboardid') . '_code_' . $schedule->get('code').'_'.$counter;                                               
                        
			//$rSelect = "<input id=".$rKEy.$rateItem['id'] ." type=\"checkbox\" name=\"$rKEy\" onclick=\"setRate(this);\" value=\"liveaboard:" . $schedule->get('id') . ":" . $rateItem['id'] . "\" />";
                        $rSelect = "<input id=".str_replace(" ","_",$rKEy).$rateItem['id'] ." type=\"checkbox\" name=".str_replace(" ","_",$rKEy).$rateItem['id']." onclick=\"setRate(this);\" value=\"liveaboard:" . $schedule->get('id') . ":" . $rateItem['id'] . "\" />";
                        
			//$boatDetailArray['ScheduleList'] .= "<td title=\"$title\" align=\"left\">$rSelect<label for=".$rKEy.$rateItem['id']."><span></span>" . number_format($price2,0) . " " . $currency . "</label></td>";
                        $boatDetailArray['ScheduleList']  .= "<td title=\"$title\" align=\"left\">$rSelect<label for=".str_replace(" ","_",$rKEy).$rateItem['id']."><span></span>" . number_format($price2,0) . " " . $currency . "</label></td>";
