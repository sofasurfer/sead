			...
			if( !empty($rateItem) ){
				...
				if( $productCodeOld != $productCode ){
					...
					if( $rateItemArray[0] == 'tour' ){
						...
						$bookingList[$productId]['name'] = $product->get('name');
						...
						$bookingList[$productId]['name'] = $product->get('name') . " - " . $product->get('boatid');
					}
...
*/
		...
		if( !empty($_GET['bid']) && $_GET['bid'] == $boatItem->get('id') ){
			...
			foreach($liveAboards as $liveAboard){
				if( $liveAboard->get('active') == 1 ){
					...
					$boatArray['SubMenuLiveaboards'] .= '<li class="'.$className.'"><a href="liveaboard-detail.html?lid='.$liveAboard->get('id').'">'.$liveAboard->get('name').'</a></li>';
				}
			}
			$boatArray['SubMenuLiveaboards'] .= '</ul>';
		}else{
	...
	foreach ($liveaboardCollection as $liveaboard){
		...
		$boatListArray[$liveaboard->get('boatid')]['liveaboard'][] = "<a href=\"[[~545]]?lid=".$liveaboard->get('id')."\">" . $liveaboard->get('name') . "</a><br/>";
	...
	$c->where(array('seadBoatSchedule.liveaboardid' => $lid ) );	
	...
	foreach( $scheduleCollection as $schedule ){
		...
		if( $boatId != $schedule->get('boatid') ){
			...
			$scheduleList .= "<a href=\"boat-detail.html?bid=".$schedule->get('boatid')."\"><h3>" . $schedule->get('boatname') . "</h3></a><table  class=\"scheduleTable\" width=\"100%\">";
	...
	foreach( $scheduleCollection as $schedule ){
		...
		if( $liveaboardId != $schedule->get('liveaboardid') ){
			...
			$boatDetailArray['ScheduleList'] .= "<a href=\"liveaboard-detail.html?lid=".$schedule->get('liveaboardid')."\"><h3>" . $schedule->get('liveaboardname') . "</h3></a><table class=\"scheduleTable\" width=\"100%\">";
		...
		$days = round($nights+1);
		$boatDetailArray['ScheduleList'] .= "<tr>";
		//$boatDetailArray['ScheduleList'] .= "<td>" . $schedule->get('code') . "</td>";
