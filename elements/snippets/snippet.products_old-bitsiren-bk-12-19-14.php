...
if( $scriptProperties['type'] == 'basket' ){
	...
	if( !empty($bookingList) && count($bookingList) > 0 ){
		foreach( $bookingList as $bookingItem ){
			...
			$retHtml .= "<table class=\"table table-condensed table-striped\">";
	...
	if( !empty($bookingList) && count($bookingList) > 0 ){
		...
		foreach( $bookingList as $bookingItem ){
			...
			$bookingForm['bookinginfo'] .= "<table class=\"table table-condensed table-striped\">";
	...
	$bookingForm['customerinfo'] = "<table class=\"table table-condensed table-striped\">";
...
*/
	...
	foreach ($liveaboardCollection as $liveaboard){
		...
		if( empty($boatListArray[$liveaboard->get('boatid')]) ){
			...
			$boatListArray[$liveaboard->get('boatid')]['boaturl'] = $urlItems[1] . "/" . $urlItems[2] . "/boats/"
	...
	foreach( $scheduleCollection as $schedule ){
		...
		if( $boatId != $schedule->get('boatid') ){
			...
			$scheduleList .= "<table  class=\"table table-condensed table-striped\" width=\"100%\">";
		...
		if( $schedule->get('boatid') == 14 ){
			...
			$scheduleList .= "<td>" . $days . " / " . $nights . "" . "</td>";
		}
	...
	$specTypeId = 0;
	$boatDetailArray['SpecList'] = "<table class=\"table table-condensed table-striped\" width=\"100%\">";
	...
	$boatDetailArray['CabinList'] = "<ul class=\"thumbnails\">";
	foreach( $cabinCollection as $cabin ){
		...
		$boatDetailArray['CabinList'] .= "<li class=\"span3\"><div class=\"thumbnail\">";
		$boatDetailArray['CabinList'] .= "<img src=\"".$cabin->get('imgthumb')."\" />";
		$boatDetailArray['CabinList'] .= "<h5>" . $cabinName . "</h5><p>" . $cabin->get('value') . "</p></div></li>";

	}
	$boatDetailArray['CabinList'] .= "</ul>";
	...
	foreach( $scheduleCollection as $schedule ){
		...
		if( $liveaboardId != $schedule->get('liveaboardid') ){
			...
			$boatDetailArray['ScheduleList'] .= "<a href=\"".$urlPath."?lid=".$schedule->get('liveaboardid')."\"><h3>" . $liveaboardname . "</h3></a><table class=\"table table-condensed table-striped\" width=\"100%\">";
	...
	$oldCat = 0;
	$hotelArray['rates'] = "<table class=\"table table-condensed table-striped\" width=\"100%\">";
	foreach( $rateCollection as $rateItem ){
