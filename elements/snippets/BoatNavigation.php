...
foreach ($boatList as $boatItem) {
    ...
    $countryArray = $modx->fromJSON($boatItem->get('countryid'));
    $boatArray['countryArray'] = $countryArray;
    
    /* noom */
    $country = $modx->getObject('seadCountry', array('id'=>$boatArray['countryArray']));    
    $boatArray['countryname'] =  strtolower($country->get('name'));
    /* noom */
    ...
    foreach ($liveAboards as $liveAboard) {
        ...
        if ($liveAboard->get('active') == 1) {
           ...
           $urlPath = "[[++site_url]]liveaboards/" . $liveAboard->get('nameurl') . ".html";
